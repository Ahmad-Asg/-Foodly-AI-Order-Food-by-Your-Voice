...
class AppShell extends StatefulWidget {
  const AppShell({super.key, required this.user, required this.api, required this.cart, required this.onLogout});
...
class _AppShellState extends State<AppShell> {
  ...
  late final List<Widget> _pages = [
    ...
    const _ComingSoonPage(icon: Icons.auto_awesome_rounded, title: 'Foodly AI', message: 'AI chat and voice will be added in a later phase.'),
    OrdersScreen(key: _ordersKey, api: widget.api),
    _ProfilePage(user: widget.user, onLogout: widget.onLogout),
  ];

  final List<String> _titles = const ['Foodly AI', 'Menu', 'Foodly AI', 'Orders', 'Profile'];
        ...
        actions: [
          AnimatedBuilder(
            ...
            builder: (_, _) => IconButton(
              tooltip: 'Cart',
              onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => CartScreen(cart: widget.cart))),
              icon: Badge(
...
class _ProfilePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SafeArea(
    child: ListView(padding: const EdgeInsets.all(24), children: [
      CircleAvatar(radius: 38, child: Text(user.name.isEmpty ? '?' : user.name.substring(0, 1).toUpperCase())),
...
class _ComingSoonPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => SafeArea(
    child: Center(child: Padding(padding: const EdgeInsets.all(32), child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 56, color: Theme.of(context).colorScheme.primary), const SizedBox(height: 16), Text(title, style: Theme.of(context).textTheme.headlineSmall), const SizedBox(height: 8), Text(message, textAlign: TextAlign.center)]))),
  );
}
